...
- [x] [Review][Defer] AC #3's literal scenario (relaunch-then-`goBack()`-into-a-dismissed-arrival-node) is not actually exercised — `resumedEngineComposesPersistedBackStackWithDismissedArrivalState` resumes the engine directly onto `.shoreArrival` and backs *away* from it, never resumes past it and backs *into* it. Tracing `StoryTree.swift`, `.shoreArrival`'s only forward target is `.endingElsewhere`, which `loadValid(from:)` unconditionally rejects — so the literal transition AC #3 names is structurally unreachable in the current placeholder content tree, not merely untested. — deferred, user decision 2026-08-03: the existing test's composition of dismissal-state + backward-stack at resume time is accepted as sufficient evidence the underlying mechanism works; revisit once Epic 4 ships a content tree with reachable post-arrival nodes.
- [x] [Review][Patch] No round-trip test for duplicate entries in `visitedNodeIds`, despite the field's own doc comment explicitly calling out that "duplicate entries... matter for a stack" (unlike the `Set`-based `visitedArrivalNodeIds` precedent) [ForkedEchoesTests/StoryRunEngineTests.swift, ForkedEchoesTests/RunSnapshotTests.swift] — fixed: added `encodeDecodeRoundTripPreservesDuplicateAndOrderedVisitedNodeIds` to `RunSnapshotTests.swift`
- [x] [Review][Patch] `resumedEngineCanNavigateBackwardThroughPersistedHistory` only asserts in-memory `currentNodeId` after each `goBack()` — it never re-reads the on-disk snapshot to confirm the shrinking stack round-trips correctly through `persistOrClearSnapshot()`, despite this story being specifically about persistence correctness [ForkedEchoesTests/StoryRunEngineTests.swift:211-245] — fixed: added a mid-sequence `RunSnapshot.loadValid(from:)` reload assertion to the existing test
- [x] [Review][Patch] No test proves `goBack()` stays blocked by `phase == .interstitial` when a resumed engine has a non-empty persisted `visitedNodeIds` stack but the arrival node hasn't been dismissed yet (force-quit mid-interstitial with prior back-history already accumulated) [ForkedEchoesTests/StoryRunEngineTests.swift] — fixed: added `goBackStaysBlockedByInterstitialGateOnAResumedEngineWithNonEmptyHistory`
- [x] [Review][Defer] `loadValid(from:)` validates `choiceHistory` against content-tree drift but adds no equivalent check for `visitedNodeIds`, an ordered stack that drives navigation directly (unlike the `Set`-based `visitedArrivalNodeIds` precedent it mirrors) [ForkedEchoes/Engine/RunSnapshot.swift:91-114] — deferred, pre-existing pattern (Dev Notes explicitly scoped this story to mirror the accepted Story 2.9 precedent; low real-world reachability since NodeID is a closed enum and drift requires a future content-tree restructuring)
- [x] [Review][Defer] No consistency check between a resumed `visitedNodeIds` stack and `currentNodeId`/`choiceHistory` — a corrupted or hand-edited snapshot could set a stack unrelated to how the player actually reached the current node [ForkedEchoes/Engine/StoryRunEngine.swift:116-118] — deferred, pre-existing; only reachable via manual `UserDefaults` tampering or future content-tree drift, no security boundary on local single-player save data
- [x] [Review][Defer] `goBack()` has no guard preventing a persisted `visitedNodeIds` entry from being an `.ending` node; normal play can never push one (`advancePage()`'s `.ending` case returns before pushing), so this is only reachable via tampered save data [ForkedEchoes/Engine/StoryRunEngine.swift:193-206] — deferred, low real-world reachability
- [x] [Review][Defer] `visitedNodeIds` grows unbounded and is re-serialized synchronously on every `selectChoice`/`advancePage`/`goBack` call, with no cap — immaterial for the current placeholder story but worth revisiting once Epic 4 ships a full-length content tree [ForkedEchoes/Engine/StoryRunEngine.swift:277-297] — deferred, not a concern at current content scale
- [x] [Review][Defer] A malformed-but-present `visitedNodeIds` key (not merely absent) throws during decode and discards the entire snapshot via `loadValid(from:)`'s catch-all, not just the one field [ForkedEchoes/Engine/RunSnapshot.swift:60-68] — deferred, pre-existing behavior inherited from the `visitedArrivalNodeIds`/`choiceHistory` pattern since Story 2.4, not introduced by this diff
...
- 2026-08-03: Code review complete (Blind Hunter, Edge Case Hunter, Acceptance Auditor). 1 decision-needed, 3 patch, 5 defer, 2 dismissed. Decision: user confirmed the existing `resumedEngineComposesPersistedBackStackWithDismissedArrivalState` test's composition of dismissal-state + backward-stack at resume time is accepted as sufficient evidence for AC #3, given the placeholder content tree can't reach the literal relaunch-then-back-into-a-dismissed-node transition. Patches applied: added `encodeDecodeRoundTripPreservesDuplicateAndOrderedVisitedNodeIds` (RunSnapshotTests.swift), a mid-sequence on-disk reload assertion in `resumedEngineCanNavigateBackwardThroughPersistedHistory`, and `goBackStaysBlockedByInterstitialGateOnAResumedEngineWithNonEmptyHistory` (StoryRunEngineTests.swift). `swift test` passing, 0 failures. Deferred items (no content-tree-drift validation for `visitedNodeIds`, no stack/currentNodeId consistency check, no ending-node guard in the stack, unbounded stack growth, whole-snapshot decode failure on a malformed field) logged to `deferred-work.md` — all low-severity, pre-existing patterns or only reachable via tampered save data. Status set to `done`.
