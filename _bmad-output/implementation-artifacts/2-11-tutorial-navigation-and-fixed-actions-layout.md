...
- [ ] Task 5: Manual verification (AC #5)
  ...
  - [ ] Record result + date in Completion Notes List once reported back
...
- Task 5: manual verification requested from user (see below) — this devcontainer has no Xcode/Simulator, so the 5-point AC #5 checklist was handed off for a real device/Simulator check rather than simulated here.
