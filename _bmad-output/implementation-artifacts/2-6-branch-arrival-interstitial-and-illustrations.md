...
- [x] [Review][Patch] Branch-arrival interstitial replays after backing up over a decided choice and paging forward again [ForkedEchoes/Engine/StoryRunEngine.swift:116,145,155,177] — fixed: `interstitialDismissed` bool replaced with a per-node `Set<NodeID>` (`dismissedInterstitialNodeIds`); regression test added (`dismissedInterstitialDoesNotReplayAfterBackingUpAndPagingForwardAgain`); 46/46 passing.
- [x] [Review][Patch] No guard against a rapid double-tap on Continue skipping past the node's reading prose [ForkedEchoes/Views/StoryChoice/BranchArrivalInterstitialView.swift:36] — fixed: local `isDismissing` state disables the button after first tap. View-layer change, no automated test coverage possible in this devcontainer (no UI test target) — needs manual double-tap check in Simulator.
- [x] [Review][Patch] Interstitial content has no scroll/overflow fallback for large Dynamic Type + landscape [ForkedEchoes/Views/StoryChoice/BranchArrivalInterstitialView.swift:19] — fixed: wrapped content in a `ScrollView`. View-layer change, no automated test coverage possible — needs manual check at largest accessibility text size + landscape in Simulator.
- [x] [Review][Patch] `BranchIllustration -> ImageResource` and `BranchIllustration -> accessibilityLabelKey` are two independently-exhaustive switches that must be kept in sync by hand [ForkedEchoes/Views/StoryChoice/BranchArrivalInterstitialView.swift:51] — fixed: collapsed into one switch returning both (`.assets`).
...
- 2026-08-02: Code review (bmad-code-review) found a real bug: the interstitial replayed after backing up over a decided choice and paging forward again, because `interstitialDismissed` was a single flag reset on every `currentNodeId` change rather than tracked per-node. Fixed via a per-node `Set<NodeID>` (`dismissedInterstitialNodeIds`); added regression test `dismissedInterstitialDoesNotReplayAfterBackingUpAndPagingForwardAgain` (46/46 passing). Also patched: a debounce guard on the Continue button (rapid double-tap could otherwise skip past the node's reading prose), a `ScrollView` fallback for large Dynamic Type + landscape, and collapsed two illustration-mapping switches into one. User confirmed all three view-layer patches in Xcode/Simulator. Status remains `done`.
