...
- 2026-08-02: UX design pass (Sally) amended UX-DR11: this story's retrofit of the run-options control onto Tutorial (Task 6, this file) is being removed — Tutorial is a pre-run explainer screen, not a page within a run, and the control duplicated/didn't fit that context (see this story's own `onRestartRun` guard against a run that might not exist yet). Removal scoped to Story 2.11; this story's own AC/implementation left historically intact. See `sprint-change-proposal-2026-08-02-tutorial-navigation-and-fixed-actions.md`.
