...
- Task 5 re-verification (2026-08-08, user-confirmed in Xcode/Simulator, post-code-review patches): all seven checks pass. (1) Tap-to-return: while receded from scrolling, tapping anywhere in the reading area (not the button) returns it to full opacity immediately, without waiting for scroll to settle. (2) Dynamic Type shrink mid-scroll: shrinking Text Size below the accessibility threshold while the button is receded (no page turn) immediately restores full opacity instead of leaving it stuck receded. (3) Button still recedes while actively scrolling, no collision with scrolled text. (4) Button still returns to full opacity on scroll-rest (no tap). (5) Tapping the button itself while receded still opens the run-options menu. (6) Reduce Motion still snaps instantly for both the scroll-rest and the new tap-to-return paths. (7) Ordinary Dynamic Type size still shows no change — button stays fully opaque.
...
- 2026-08-08 (code review): Two findings patched. (1) `StoryChoiceView.swift` — added `.onChange(of: dynamicTypeSize.isAccessibilitySize)` to reset `isReadingContentScrolling` when a live Dynamic Type shrink swaps `readingComposition` out of the ScrollView branch without a page turn, closing an AC #5 gap the existing `.onChange(of: engine.currentNodeId)` reset didn't cover. (2) `LayoutMetrics.swift`'s `accessibilitySizeFramedScroll()` — added a `.simultaneousGesture(TapGesture())` on the ScrollView to reset `isScrolling` on any tap within the reading area, implementing DESIGN.md `components.run-options-button`'s second return-trigger ("scroll comes to rest, or any tap within the reading area") that Task 2's original implementation only partially covered. Both changes touch `.swift` code post-Task-5-verification — per this story's own Testing Standards Summary, status stays at `review`; Task 5's manual checks (specifically: tap-to-return while receded, and Dynamic Type shrink mid-scroll) need to be re-run in Xcode/Simulator before advancing to `done`.
- 2026-08-08: Task 5 re-run in Xcode/Simulator against the patched code, user-confirmed — all seven checks pass (see Completion Notes). Status moved to `done`.
