...
- **[RESOLVED — Story 3.5 audit, 2026-08-07]** ~~EndingView's swipe-back gesture may be unreachable at accessibility Dynamic Type sizes~~ — confirmed via code inspection: `EndingView.swift`'s outer `.gesture(backSwipeGesture)` (line 49) is attached above/outside the `GeometryReader`/`ScrollView` that `.accessibilitySizeFramedScroll()` mounts only at accessibility Dynamic Type sizes (`content`, lines 81-94), so the structural conflict the original review flagged is real and unchanged as of this audit. Actual touch-arbitration outcome (does the `ScrollView`'s `UIPanGestureRecognizer` swallow the swipe) requires a live AX5 Simulator test per project-context.md's SwiftUI gesture arbitration section — this devcontainer cannot render/exercise it. Flagged to the user as an explicit Simulator checkpoint in Story 3.5's Completion Notes; if confirmed unreachable, treat as a fast-follow candidate (Story 3.8 or a new entry), not a silent scope expansion of 3.5. **Note, 2026-08-07 (Sprint Change Proposal):** this item is unaffected by the VoiceOver scope decision — the swipe-back reachability question is about touch/gesture arbitration (`ScrollView` vs. sibling `.gesture()`), not VoiceOver, and remains a real open Simulator checkpoint for Story 3.5.
