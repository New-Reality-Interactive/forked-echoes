...
- **[RESOLVED — tracked as Story 3.5, 2026-08-06]** ~~EndingView's swipe-back gesture may be unreachable at accessibility Dynamic Type sizes~~ — `EndingView.swift`'s outer `.gesture(backSwipeGesture)` sits outside the `GeometryReader`/`ScrollView` that only activates at accessibility sizes, and a `ScrollView`'s own drag recognizer typically wins over a sibling `.gesture()` covering the same region. Now an explicit AC on Story 3.5 (End-to-End Accessibility Validation).
...
- **[RESOLVED — tracked as Story 3.5, 2026-08-06]** ~~Non-text color pairs (the secondary-button border stroke, Memory's row divider fill) are not covered by DESIGN.md's text-contrast table~~ — these are graphical-object contrast (WCAG 1.4.11), not the text contrast (1.4.3) the table documents, so they were never formally re-verified during Story 3.4's Task 3 pass. Now an explicit AC on Story 3.5.
...
- **[RESOLVED — tracked into Story 4.2, 2026-08-06]** ~~No VoiceOver-specific alternative wording for the gesture-only page-turn mechanic~~ — now an explicit AC on Story 4.2 (Full Prose Authoring): Tutorial's final `tutorial.mechanic.pageTurn` copy must also mention the VoiceOver rotor "Next Page"/"Previous Page" alternative (Story 2.2/UX-DR12), not just swipe/tap-edge wording.
...
- **[RESOLVED — folded into Story 3.5, 2026-08-06]** ~~VoiceOver "Next Page"/"Previous Page" custom actions stay exposed and silently no-op when blocked~~ — the `.ending` placeholder this was originally observed on no longer exists (replaced by the real `EndingView`, Story 3.2). Now an explicit audit item on Story 3.5.
...
- **[RESOLVED — folded into Story 3.5, 2026-08-06]** `runOptionsButtonClearance` unverified at AX5 — now an explicit AC on Story 3.5.
