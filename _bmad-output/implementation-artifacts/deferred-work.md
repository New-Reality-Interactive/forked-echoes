...
**Status as of 2026-08-06: every entry below is closed — resolved in code/docs, dismissed as accepted/non-issue with rationale, or tracked into a specific story. Nothing in this file represents open, un-owned work.** New entries get appended above as future reviews surface them; each new entry should reach one of those three end states before this file is next audited, not accumulate indefinitely.
...
- **[OPEN — needs UX design input]** `runOptionsButtonClearance` overlap confirmed real at AX5: the run-options button's ellipsis glyph grows with Dynamic Type (no `.font()`/`.imageScale()` cap on `RunOptionsButton.swift`'s `Image(systemName: "ellipsis.circle")`) while the reserved clearance (`LayoutMetrics.runOptionsButtonClearance`, 60pt fixed) does not, so at AX5 the enlarged button sits over reading text as the user scrolls up the accessibility-size `ScrollView`. User-confirmed via Simulator, 2026-08-08 (Story 3.5's AC7 checkpoint) — functionally still usable (not blocking), but visually wrong. No fix applied yet; the right placement/behavior for the run-options button at accessibility Dynamic Type sizes needs UX design input (Sally) before implementing — candidates include capping the glyph's `.imageScale()`, scaling the clearance to track Dynamic Type, or repositioning the button entirely at accessibility sizes. Track as a fast-follow story once a design direction is picked.

## Deferred from: code review of story-3-5-end-to-end-accessibility-validation (2026-08-08)

- **[OPEN — pre-existing]** `HomeView`/`TutorialView` action buttons (`HomeView.swift:41-51`, `TutorialView.swift` equivalent) share the same unpadded `.frame(maxWidth: .infinity, minHeight: LayoutMetrics.minTapTarget)` pattern this review's fix addressed on the interstitial Continue button — at AX5 the button grows to fit its text with no explicit padding around it. Not confirmed broken: this story's AX5 sweep checked for clipped/truncated text, not specifically for missing breathing room, so this latent variant of the same bug class was not directly re-verified.
- **[OPEN — introduced by this diff, unverified]** New `.padding(.vertical, Spacing.small)` on the interstitial Continue button (`BranchArrivalInterstitialView.swift:129-131`) adds to the required content height in the non-scrolling compact-height (landscape) layout path at ordinary Dynamic Type, which has no `ScrollView` escape route. Not re-checked against `illustrationMaxHeightFractionCompact`'s headroom in that specific orientation/size combination.
- **[OPEN — pre-existing]** Decided-but-not-selected `ChoiceCardView` cards keep the `.isButton` trait and a live (no-op) `.accessibilityAction(.default, …)` (`ChoiceCardView.swift:148-149`) with no disabled/`.notEnabled` trait to signal the card is inert.
- **[OPEN — pre-existing]** EXPERIENCE.md's exact VoiceOver wording contract is still not literally matched: no "Choice" role prefix in the accessibility label, and the pre-existing (Story 2.3) undo-window hint string doesn't mention "double-tap"/"1.5 seconds" (`ChoiceCardView.swift:148,154`; `Localizable.xcstrings:595-603`). This story's Completion Notes describe the new value strings as "restoring" the Accessibility Floor example, which overstates what changed.

## Deferred from: code review of 3-6-reading-and-ending-surface-background-and-frame-inset-rule (2026-08-06)

- `isFrameEligibleNode == false` reading nodes render on `surfaceBase` directly with no `surfaceRaised` card fill (no `FrameView` attached there) — pre-existing gating logic unchanged by this story, not a regression; matches Memory's own no-raised-card precedent.
- `LayoutMetrics.swift`'s file header ("Named layout/opacity constants... Pure refactor", Story 1.6) is stale relative to the file's actual contents — it already held non-constant `extension View` helpers before this story added `GeometryProxy.symmetricSafeAreaInset`, another non-constant computed property.
- `FrameView`'s `.background` attachment (this story's fix for the `.overlay` text-hiding bug) silently depends on `content` at every call site being sized to exactly the bounds `FrameView`'s own `GeometryReader` resolves to, with no compiler or test enforcement of that invariant — architectural risk inherent to the pattern, not solvable without UI/snapshot test infrastructure this project doesn't have.
- No automated regression test (snapshot/reference-image) guards the accessibility-size scroll/safe-area geometry despite its five-round manual-debugging history during this story's Task 4 — consistent with this project's existing zero-view-test convention (AD-7), not a gap specific to this story.
