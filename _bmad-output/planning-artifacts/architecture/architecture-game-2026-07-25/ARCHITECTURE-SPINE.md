...
- **Rule:** One `@Observable` `StoryRunEngine`, injected via `@Environment`, is the sole mutator of run state. Its full intent surface: `selectChoice(_:)`, `advancePage()` (also serves as "Continue" past the branch-arrival interstitial and "tap to continue" past Ending — the same "move forward past the current blocking beat" intent in all three phases), `goBack()`, `exitToHome()` (non-destructive, preserves the snapshot), `restartRun()` (mid-run, destructive, requires the confirmation EXPERIENCE.md specifies), `exitAndClearProgress()` (Story 2.13, mid-run, destructive, requires its own confirmation — same field reset as `restartRun()`, but clears rather than re-persists the on-disk `RunSnapshot` so it also leaves the run instead of resetting in place), `startNewRun()` (from Memory, post-completion, no confirmation needed). Views never write engine state directly. Every interaction path that expresses the same user intent — gesture, its accessible tap equivalent, and its VoiceOver custom action alike (FR-11) — invokes the *same* intent method; these are never separate code paths that could diverge.
...
- **Rule:** `RunSnapshot` is `Codable`, JSON-encoded, stored under one `UserDefaults` key: `currentNodeId: NodeID`, `choiceHistory: [ChoiceRecord]` (each a `{ nodeId: NodeID, chosenOptionId: OptionID }` pair — IDs only, never frozen prose; Memory re-resolves display text from the current String Catalog via those IDs at render time per AD-2, so a content edit between builds never strands stale text in an old snapshot), `alignmentScore: Int`, `tutorialSeen: Bool`, and (Story 2.9) `visitedArrivalNodeIds: Set<NodeID>` — branch-arrival nodes whose forced-Continue gate has been dismissed, needed so AD-5's first-visit-ever gate survives a relaunch (decodes to `[]` for any snapshot written before Story 2.9, which has no such key).
...
- **Binds:** navigation, RootView, StoryRunEngine, Story/Choice view, FR-3
- **Prevents:** forcing tree-shaped, choice-gated content into `NavigationStack` push/pop or `TabView(.page)` — both assume a fixed, known page list and have no clean hook to block forward navigation on an external condition. Also prevents the Story session's own full-surface gesture language (swipe/tap-zone page-turning) from ever having to compete with system navigation chrome — a `NavigationStack` push's back button and `interactivePopGestureRecognizer` swipe gesture, which real Simulator testing (Story 2.2) showed wins over an in-surface `DragGesture` unconditionally, not just at the screen edge.
- **Rule:** `StoryRunEngine` owns "what is currently displayed" as a phase, **derived from the current node's type** (not decided by a separate look-ahead step) — reading a node / branch-arrival interstitial / ending / memory; see the state diagram below. Because phase is derived, a hard-fail transitions to Ending the instant `selectChoice(_:)` targets an Ending-kind node, not on some later `advancePage()` discovery. Swipe gestures and the invisible tap zones both call `advancePage()` / `goBack()`, and are only attached to the reading surface — the interstitial exposes no swipe/tap-zone recognizers at all *on a node's true first visit*, responding only to its own tap-to-continue affordance (`EXPERIENCE.md`'s "blocks page-turn gestures until dismissed"). The branch-arrival illustration+caption is the arrival node's permanent page content — there is no separate "ordinary reading content" to fall back to. Phase derives to `.interstitial` (forced-Continue, no swipe/tap-zone) only on a node's true first-ever visit, keyed off `RunSnapshot.visitedArrivalNodeIds` (a fifth `RunSnapshot` field added by Story 2.9 specifically for this — not, as an earlier draft of this amendment claimed, an existing field; see that story's Dev Notes for the discrepancy this corrects) rather than a session-only flag — so the gate correctly reflects first-visit-ever, not merely first-visit-this-session. Every subsequent visit — backing up to the node, or arriving again after an app relaunch — renders the identical illustration+caption but behaves as an ordinary swipeable page (`advancePage()`/`goBack()` behave normally, no Continue button needed). The engine decides whether a reading transition is allowed (blocks forward on an unresolved choice; shows a revisited choice locked, per FR-5) and the view transitions its content via `.transition`/`.animation` in response. `NavigationStack` is reserved for Home ↔ Tutorial's back-and-forth wayfinding only. The Story session — every engine-driven phase in the state diagram below (Reading, Interstitial, Ending, Memory) — is presented from Home or Tutorial via `.fullScreenCover`, not pushed onto the `NavigationStack`, and never wraps individual story pages either way.

  *(Amended 2026-07-31, Story 2.2. Original wording — "`NavigationStack`, if used at all, is reserved for the coarse top-level flow (Home ↔ Tutorial ↔ Story session ↔ Ending ↔ Memory)" — implied the Story session was just one more `NavigationStack` destination alongside Home/Tutorial. Real Simulator testing showed why that's wrong: a `NavigationStack` push always carries a system back button and an `interactivePopGestureRecognizer` swipe gesture, and the latter won unconditionally over the Story session's own swipe-left/right page-turn `DragGesture`, regardless of where the drag started on screen. A UIKit-interop attempt to selectively disable that system gesture (mirroring `ColdLaunchOrientationFix.swift`'s precedent) proved unreliable and unverifiable without a live device. `.fullScreenCover` has no back button and no swipe-to-dismiss by construction, so there is nothing to suppress — it also matches the intent already flagged in `deferred-work.md`'s Epic 1 follow-up, that AD-5 "explicitly avoids relying on default back-gesture chrome," far more literally than the original wording delivered. The only sanctioned exit from the Story session is a deliberate action — Memory's "Return Home," or a mid-run "Exit to Home" (Story 2.7's run-options sheet) — never an incidental system gesture.)*

  *(Amended 2026-08-02, Story 2.9. Story 2.6's original interstitial design was a one-shot, non-persisted gate: the arrival node showed the illustration+caption exactly once, gated by a forced Continue tap, then permanently swapped to the node's ordinary body prose — a swap keyed off an in-memory-only flag that didn't survive an app relaunch either. Simulator playtesting (2026-08-02) showed this loses the branch-arrival moment from the story's path entirely after the first pass, which didn't match the intended feel of a real "place" in the story. Revised: the illustration+caption is now the node's permanent content, and the forced-Continue gate is keyed off `RunSnapshot.visitedArrivalNodeIds` so it reflects true first-visit-ever rather than first-visit-this-session.)*

  *(Correction, 2026-08-02, Story 2.9 implementation. This amendment's first pass (above) and the Sprint Change Proposal it was drafted from both described the gate as keyed off an "already-persisted `visitedNodeIds`" — that claimed no new persisted state was needed. Reading the actual codebase at implementation time showed this was inaccurate: `StoryRunEngine.visitedNodeIds` is an unpersisted, session-only back-navigation stack, and the field that actually gated the interstitial (`dismissedInterstitialNodeIds`) was also unpersisted. No field satisfying "survives relaunch" existed. Story 2.9 therefore added a genuinely new fifth `RunSnapshot` field, `visitedArrivalNodeIds: Set<NodeID>`, populated whenever `advancePage()` dismisses the interstitial and read back by `resumingFromSnapshot(defaults:)`. This is a real, deliberate exception to `RunSnapshot`'s previously-exhaustive four-field shape (AD-4) — not the zero-schema-change refactor originally planned. Recorded here per this project's "most recent explicit decision wins" convention (project-context.md).)*
