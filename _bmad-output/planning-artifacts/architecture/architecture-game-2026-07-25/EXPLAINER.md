...
## Landscape reflows the same views; it doesn't fork them (AD-8)

When landscape support was added (Sprint Change Proposal, 2026-07-26), the tempting shortcut was a size-class check using `horizontalSizeClass` — that's the size class most SwiftUI tutorials reach for first. It doesn't work here: on every iPhone except the largest Plus/Max models, `horizontalSizeClass` reports `.compact` in *both* portrait and landscape, so it can't actually tell the two apart. `verticalSizeClass` is the one that flips (`.regular` in portrait, `.compact` in landscape) consistently across the whole iPhone lineup, which is why AD-8 pins to it specifically rather than leaving "which size class" as an implementation detail every future screen-building story would have to rediscover on its own.

The other shortcut worth naming and rejecting: reaching for `UIDevice.orientation` / `UIDeviceOrientationDidChangeNotification` directly. That's UIKit-era imperative API — it needs a subscribed observer and manual teardown, and it doesn't correspond 1:1 with interface orientation (a device lying face-up or face-down reports its own dedicated `.faceUp`/`.faceDown` case, neither of which maps to any of the four rotations a layout would actually need to react to). SwiftUI's environment-based size classes sidestep all of that for free.

The deeper reason landscape didn't need its own parallel view hierarchy at all: nothing about *what* a screen shows changes with orientation, only how much horizontal room it has to lay it out in. A `LandscapeHomeView` alongside `HomeView` would just be the same content duplicated with a slightly different `VStack`/`HStack` — the kind of duplication that drifts the moment one gets a content or copy fix and the other doesn't. One view, branching internally on `verticalSizeClass` only where the layout genuinely needs to (choice cards stacking vs. rowing), keeps that single-source-of-truth property landscape support shouldn't have cost in the first place.
