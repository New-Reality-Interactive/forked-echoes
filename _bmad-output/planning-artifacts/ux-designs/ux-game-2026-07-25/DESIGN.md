...
typography:
  headline:
    ...
    note: 'Uppercase, with one carved-out exception: Home/Tutorial''s story-title use of this role renders Title Case, per the home.html/tutorial.html mockups and Story 1.4''s implementation (2026-07-26) — the mockups were followed over this note for that specific use. Other headline-role usages (e.g. `{typography.stat}` on the Memory screen) remain Uppercase per this rule until their own story explicitly revisits it. Base size at default Dynamic Type category — scales with system setting (FR-11).'
  body:
    fontFamily: 'SF Pro (system sans)'
    fontSize: 20px
    fontWeight: '500'
...
- **Home / Tutorial chrome** — Home and Tutorial are title-card and instructional surfaces, not reading pages. Neither carries the circuit frame — the frame is reserved for reading surfaces (Story/Choice, Ending); Home and Tutorial share the same simpler, more spacious layout so the frame's appearance always means "you're inside the story." (Corrected 2026-07-26: this previously listed Tutorial among the frame-bearing surfaces, contradicting Story 1.4's resolved decision — see UX-DR1 in `epics.md` and `deferred-work.md`'s "Follow-ups from: sprint demo" section.)
...
Reference mockups: [`mockups/story-choice-warm-ink-circuit.html`](./mockups/story-choice-warm-ink-circuit.html) (story/choice screen, echo state, branch-arrival interstitial), [`mockups/story-choice-three-way.html`](./mockups/story-choice-three-way.html) (3-choice decision point — 2 ordinary + 1 hard-fail — at default and accessibility-Dynamic-Type sizes), [`mockups/home.html`](./mockups/home.html) (fresh-install and run-in-progress/"Resume Story" states), [`mockups/tutorial.html`](./mockups/tutorial.html) (no circuit frame, matches Home's chrome; run-options icon), [`mockups/ending.html`](./mockups/ending.html) (permanently-active frame; home vs. hard-fail variants side by side), [`mockups/memory.html`](./mockups/memory.html) (score/tier header, choice-and-consequence rows, no frame).

**Landscape mockups** (Epic 5, Story 5.1): [`mockups/home-landscape.html`](./mockups/home-landscape.html) (fresh-install and run-in-progress states, reflowed), [`mockups/tutorial-landscape.html`](./mockups/tutorial-landscape.html) (no circuit frame, matches Home; reading column capped/centered), [`mockups/story-choice-landscape.html`](./mockups/story-choice-landscape.html) (2-choice and 3-choice states at default text size; the accessibility-Dynamic-Type 2+1 wrap case is deferred to Story 5.3). Branch-arrival interstitial, Ending, and Memory landscape mockups are intentionally deferred to their own future epics, informed by these.
...
| Reserve the circuit frame for reading surfaces (Story/Choice, Ending) | Put the frame on Home or Tutorial — it should always mean "you're inside the story" |
