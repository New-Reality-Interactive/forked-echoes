...
| Tutorial actions | Tutorial | "Back home" / "Start Story", tap only — tutorial is not itself gestural, it *describes* the gesture (including the page-turn tap zones and choice-card tap/hold, so the player knows both exist before reaching a real choice). |
