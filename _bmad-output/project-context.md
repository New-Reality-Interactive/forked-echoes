...
Last Updated: 2026-07-31 (Story 2.2: AD-5 amended — the Story session is presented from Home via `.fullScreenCover`, not a `NavigationStack` push; see this file's Navigation section and `ARCHITECTURE-SPINE.md`#AD-5 for the full rationale. Story 2.1's `Package.swift`/Environment-section rules from earlier the same day still hold unchanged — see `2-1-minimal-story-content-and-engine-foundation.md` and `2-2-page-navigation.md` for the full narrative history of each.)
